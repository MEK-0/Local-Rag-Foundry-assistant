...
def compress_context_chunks(query: str, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for chunk in chunks:
        # SAFEGUARD: If cover/first pages are loaded, strip formatting noise and keep pure lines
        if chunk.get("page_number") in [1, 2, "1", "2"]:
            lines = chunk["chunk_text"].split("\n")
            clean_lines = []
            for line in lines:
                line_strip = line.strip()
                # Skip legal boilerplate noise or document serial numbers to prevent LLM distraction
        ...
        # Standard context windowing for deep page assets
